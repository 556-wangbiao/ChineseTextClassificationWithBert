...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        truncated_text = self.all_text[index][:self.max_len-1] # 截断max_len-1是为了给CLS留一个位置
        label = self.all_label[index]

        # 分词
        tokenized_text  = self.tokenizer.tokenize(truncated_text)

        # 编码
        token_id = self.tokenizer.convert_tokens_to_ids(["[CLS]"] + tokenized_text)
        # 掩码  
...
if __name__ == "__main__":
    train_text, train_label, max_len = read_data("./my_data/train.txt")
    max_len = 512
